...
def seq2num(string):
    if type(string) == str:
        return np.array([aadict[x] for x in string], dtype=np.int16)[np.newaxis,:]
    elif type(string) ==list:
        return np.array([[aadict[x] for x in string_] for string_ in string], dtype=np.int16)
...
# parallel and optimized routine to count neighbours - much faster than the standard one
from numba import njit, prange
@njit(parallel=True,cache=True,nogil=False)
def count_neighbours_parallel(MSA, threshold = 0.1): 
    B = MSA.shape[0]
    N = MSA.shape[1]
    num_neighbours = np.zeros(B)
    for a in range(0, B):
